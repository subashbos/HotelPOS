...
{
    public class UpdateEstimationCommandValidator : AbstractValidator<UpdateEstimationCommand>
    {
        public UpdateEstimationCommandValidator()
        {
            RuleFor(x => x.Estimation).NotNull().WithMessage("Estimation data cannot be null.");

            RuleFor(x => x.Estimation.Id)
                .GreaterThan(0).WithMessage("A valid estimation ID is required.")
