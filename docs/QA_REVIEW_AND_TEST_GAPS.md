...
8. ~~Add real HTTP-level integration tests for controllers that only had mocked-service tests.~~ DONE (2026-08-04) — `CashSessionsHttpTests.cs`, `SettingsHttpTests.cs`, `UsersHttpTests.cs`, and `PayrollHttpTests.cs` exercise `CashSessionsController`, `SettingsController`, `UsersController`, and `PayrollController` through the real ASP.NET pipeline (`CustomWebApplicationFactory`: real routing, model binding, JWT auth, RBAC, and the mediator/EF pipeline against an isolated in-memory SQLite DB), following the pattern `RoleAuthorizationTests.cs` established. These catch classes of bug mocked-service controller tests structurally can't: routing/model-binding mismatches, JSON (de)serialization shape, and real DB-backed permission wiring (e.g. `SettingsController.GetSettings`'s permission-conditional field visibility). Remaining controllers without real HTTP-level tests: `AttendanceController`, `AuditController`, `CategoriesController`, `CustomersController`, `EmployeesController`, `EssController`, `ExpensesController`, `LeaveController`, `PurchasesController`, `RolesController` (partially covered via `RoleAuthorizationTests.cs`), `SuppliersController`, `TablesController`, `TdsController`, `UnitOfMeasurementsController`.
