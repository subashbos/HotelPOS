...
8. `POST /api/purchases` with an invalid `SupplierId` (e.g. `0`) returned `500` instead of `400`. (FIXED 2026-08-04)
   ...
   - Audited all 17: 8 confirmed as the same live gap, patched with the narrow pattern; the rest have only trivial (`id > 0`) validators fully shadowed by a controller `if (id <= 0)` guard, or are already re-validated elsewhere, or unreachable. (FIXED 2026-09-03)
     - `EstimationService.SaveEstimationAsync`/`UpdateEstimationAsync` — nothing validated status strings, item quantity/price, or a fully-discounted `GrandTotal` before this. Now validates via `new SaveEstimationCommandValidator()`/`new UpdateEstimationCommandValidator()` at the top of each method (both constructor paths, since neither one validated before).
     - `ReservationService.SaveReservationAsync`/`UpdateReservationAsync` — the `EndTime > StartTime` and `PartySize > 0` rules existed nowhere else. Same pattern as Estimation.
     - `SettingService.SaveSettingsAsync` — the handler had zero checks; an empty `HotelName` (printed on every receipt) wrote through unchecked. Same pattern.
     - `CategoryService.UpdateCategoryAsync`, `UnitOfMeasurementService.UpdateUnitAsync`, `TableService.UpdateTableAsync` — cheapest fix: each service already held the right validator instance for its legacy repository path, but `if (_mediator != null) { ...; return; }` skipped over it. Moved the existing `.Validate(...)` call above that branch so it applies to both paths.
     - Surfaced the same test-fixture blind spot as `UpdateOrderCommand`: `EstimationServiceTests`/`ReservationServiceTests` had `Estimation`/`Reservation` objects missing `GrandTotal`/`EstimationDate`/`EndTime`-`StartTime` — fixed.
     - Regression tests: `EstimationsHttpTests.CreateEstimation_ZeroQuantityItem_ReturnsBadRequest`, `ReservationsHttpTests.CreateReservation_EndTimeBeforeStartTime_ReturnsBadRequest`, `SettingsHttpTests.SaveSettings_EmptyHotelName_ReturnsBadRequest`, `CategoriesHttpTests.UpdateCategory_EmptyName_ReturnsBadRequest`, `UnitOfMeasurementsHttpTests.UpdateUnitOfMeasurement_EmptyName_ReturnsBadRequest`, `TablesHttpTests.UpdateTable_ZeroNumber_ReturnsBadRequest`.
     - Not patched (verified safe): the 8 `Delete*Command`s and `DeleteEstimationCommand`/`DeleteReservationCommand` (trivial `id > 0` validators, fully shadowed by each controller's own `if (id <= 0)` guard); `RefundOrderCommand` (triple-guarded — controller, `OrderService.RefundOrderInternalAsync`, and `RefundItemsAsync` all re-check). `DeleteOrderCommand` is dead code — `OrdersController` has no DELETE route and `OrderService.DeleteOrderAsync` calls `DeleteOrderInternalAsync` directly, never through the mediator; only unit tests construct it.
   - **Root-caused and fixed 2026-09-03**: `ValidationBehavior<TRequest, TResponse>` declared `where TRequest : IRequest<TResponse>`. In MediatR 14.1.0, a void `IRequest` command does **not** implement `IRequest<Unit>` — it only implements the non-generic `IRequest`/`IBaseRequest` markers (confirmed via reflection: `typeof(SavePurchaseCommand).GetInterfaces()` returns `IRequest, IBaseRequest, IEquatable<...>`, nothing generic). MediatR's own `IPipelineBehavior<TRequest, TResponse>` has **no constraint** on `TRequest` at all. So for any void command, the .NET DI container can never construct the closed generic `ValidationBehavior<TRequest, Unit>` (`TRequest` fails the `IRequest<Unit>` constraint) — `GetServices<IPipelineBehavior<TRequest, Unit>>()` silently returns zero results instead of throwing, and `next()` runs unvalidated. Proven with a standalone repro sending `SavePurchaseCommand` directly through `IMediator` (bypassing every service-layer hand patch): before the fix the handler ran straight into an in-memory-DB transaction error; after changing the constraint to `where TRequest : notnull`, the same call correctly threw `FluentValidation.ValidationException`.
   - This one-line constraint change makes the pipeline validate **every** void command with a validator, everywhere (API and WPF) — not just the 8 already patched. It also surfaced a 9th real, previously-unknown gap the 17-command audit didn't catch: `CashService.CloseSessionAsync`'s mediator branch had no negative-`ActualCash` guard of its own (the existing `CashServiceTests` regression only exercises the legacy non-mediator constructor path) — the pipeline now catches it directly, no service-layer patch needed.
   - Verified against the full test suite: `dotnet test src/HotelPOS.Api.Tests` — 1,554 passed, 0 failed (no regressions; the already-patched commands' explicit `.Validate()`/`ValidatePurchase()` calls just never get a chance to be redundant, since they still run first).
   - Status: FIXED (`ValidationBehavior.cs`). The narrow per-command patches above (Purchase, Order, Category, Table, UnitOfMeasurement, Estimation, Reservation, Settings) are now provably redundant defense-in-depth rather than load-bearing, but were left in place rather than removed as part of this fix — that's a separate, lower-value cleanup with its own risk (their hand-rolled checks throw different exception types than FluentValidation in a couple of spots, worth checking test assertions before touching).
...
10. `HotelPOS.WPF` never ran *any* FluentValidation validator through MediatR - a bigger version of item 8's gap, not just the void-command edge case. (FIXED 2026-09-03)
    ...
    - Note: at the time this fix was written, this item did not touch the underlying `ValidationBehavior` pipeline-behavior gap for void commands noted in item 8 - that was a separate, still-open question. It has since been root-caused and fixed directly in `ValidationBehavior.cs`; see item 8's "Root-caused and fixed 2026-09-03" note. This item remains correct on its own: the desktop app wasn't wired up to use the pipeline (registration + validators) at all, independent of the item-8 constraint bug.
